...
- Compose：开发、生产、测试 profile config 通过；`compose-smoke.sh` 实际启动 backend/frontend/PostgreSQL/Redis/mock-provider/HTTPS gateway，等待真实 health，执行合法/非法 Origin 创建 smoke，并验证 external reservation 为 0 后清理。核心服务只连 internal 网络，gateway 单独连接 edge；`DENY_EXTERNAL_NETWORK=1`，无真实 provider。GitHub Actions run `31243236261`（commit `68e312787f4c476e3796f38e86d89ab3cf5d1b94`）中 frontend 的 Vitest、typecheck 与 production build 通过，但 job `93067494916` 的 bridge frontend matrix 因未安装 Playwright Chromium 可执行文件而失败；full-stack-e2e 因已有显式 Chromium 安装步骤而通过。当前 workflow 已在 frontend job 复用同一锁文件对应安装命令，远端复跑尚未发生，修复结果仍为 unknown；失败日志经脱敏过滤后采集。
